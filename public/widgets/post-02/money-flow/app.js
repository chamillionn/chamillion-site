...
(function () {
  ...
  function resize() {
    ...
    BH  = Math.max(Math.min(H * 0.52, 80), 62);
    BW  = BH * 1.7;
    GAP = BW * 0.30;
    CY  = H / 2;
  }
  ...
  for (var i = 0; i < 4; i++) {
    blocks.push({ sealed: true, sealAnim: 1, linkAnim: 1, data: makeBlockData() });
  }
  blocks.push({ sealed: false, sealAnim: 0, linkAnim: 0, data: makeBlockData() });

  // ── Block X position ──
  function blockX(idx) {
    var stride = BW + GAP;
    var totalW = blocks.length * BW + (blocks.length - 1) * GAP;
    return (W - totalW) / 2 + idx * stride + offsetX;
  }
  ...
  function drawBlock(x, y, block, fillP) {
    ...
    if (block.sealed) {
      ...
      ctx.fillStyle   = p.label;
      ctx.fillText(block.data.txCount + ' tx', lx + 9, ty + BH - 12);

      // Gas right
      ctx.textAlign   = 'right';
      ctx.fillStyle   = p.labelDim;
      ctx.fillText(block.data.gas + ' gwei', lx + BW - 9, ty + BH - 12);
      ...
      ctx.textAlign   = 'center';
      ctx.fillText('PENDIENTE', x, ty + 18);

      // Fill progress bar
      var barX = lx + 9, barY = ty + BH - 18;
      var barW = BW - 18, barH = 3;
      ctx.globalAlpha = 0.5;
      ctx.fillStyle   = p.fillBar;
      ctx.beginPath(); ctx.roundRect(barX, barY, barW, barH, 2); ctx.fill();
      ctx.globalAlpha = 0.85;
      ctx.fillStyle   = p.fillBarFg;
      ctx.beginPath(); ctx.roundRect(barX, barY, barW * fillP, barH, 2); ctx.fill();
      ...
      if (fillP > 0.1) {
        ...
        ctx.fillText(txPreview + ' tx…', x, ty + BH - 24);
      }
    }
  ...
  function drawLink(x1, x2, sealed, sealP, linkP) {
    ...
    if (sealed) {
      ...
      ctx.beginPath(); ctx.arc(sx + 1, CY, 2.2, 0, Math.PI * 2); ctx.fill();

      // Right dot only when link fully drawn
      if (linkP > 0.92) {
        var dotAlpha = (linkP - 0.92) / 0.08;
        ctx.globalAlpha = dotAlpha * (0.4 + sealP * 0.45);
        ctx.beginPath(); ctx.arc(ex - 1, CY, 2.2, 0, Math.PI * 2); ctx.fill();
      }
      ...
      if (linkP > 0 && linkP < 1) {
        ...
        var gHead = ctx.createRadialGradient(headX, CY, 0, headX, CY, 7);
        ...
        ctx.beginPath(); ctx.arc(headX, CY, 2, 0, Math.PI * 2); ctx.fill();
      }
    } else {
  ...
  function tick(ts) {
    ...
    if (phase === 'pulsing' && phaseTime >= 0.9) {
      ...
      blocks.push({ sealed: false, sealAnim: 0, linkAnim: 0, data: newData });
