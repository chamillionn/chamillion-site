...
export default function Home() {
  ...
  // Close active card when tapping outside on mobile (permanent listener + ref)
  useEffect(() => {
    function onTouch(e: TouchEvent) {
      if (!activeCardRef.current) return;
      const target = e.target as HTMLElement;
      if (!target.closest(".nav-card")) {
      ...
      <div className="landing">
        ...
        <nav className="nav-block">
          <div className="nav-cards">
            <div
              className={`nav-card${activeCard === "newsletter" ? " nav-card--active" : ""}`}
              onMouseMove={handleCardMouse}
              onMouseLeave={() => setAccent("default")}
              onTouchEnd={(e) => {
                if (activeCardRef.current !== "newsletter") {
                  e.preventDefault();
                  activeCardRef.current = "newsletter";
            ...
            <div
              ...
              onMouseLeave={() => setAccent("default")}
              onTouchEnd={(e) => {
                if (activeCardRef.current !== "hub") {
                  e.preventDefault();
                  activeCardRef.current = "hub";
                  setActiveCard("hub");
                  setAccent("hub");
                }
              }}
